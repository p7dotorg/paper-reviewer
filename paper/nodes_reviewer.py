...
_TOOL_INSTRUCTIONS = {
    "citations": (
        ...
        "• search_papers(query) — busca no Semantic Scholar (cobertura ampla: CS, psicologia, filosofia, medicina, ciências sociais)\n"
        "• get_paper(arxiv_id) — lê o abstract de um paper arXiv (só use se o resultado tiver ID arXiv)\n"
        "• verify_doi(doi) — confirma via Crossref quando o DOI está explícito no texto\n\n"
        "Estratégia: verifique ≥5 referências — priorize títulos vagos ou autores desconhecidos.\n"
        "Para cada referência: search_papers com 'autor ano título-parcial' (ex: 'Flavell 1979 metacognition').\n\n"
        "REGRA CRÍTICA — 'não encontrado no arXiv' NÃO é evidência de alucinação:\n"
        "  • Papers de psicologia, filosofia, medicina, ciências sociais raramente estão no arXiv\n"
        "  • Papers pré-2000 também raramente estão no arXiv independente da área\n"
        "  • Antes de marcar como suspeito: tente 2 variações da query no search_papers\n"
        "  • Só marque NÃO ENCONTRADO se: (a) não achou no Semantic Scholar com ≥2 queries E\n"
        "    (b) o título/autor parece implausível (genérico, inconsistente, autor inexistente na área)\n\n"
        "FORMATO OBRIGATÓRIO — cada finding DEVE conter o título verificado:\n"
        "  • VERIFICADO: [título] — encontrado no Semantic Scholar, conteúdo bate com o citado\n"
        "  • MISMATCH: [título] — encontrado mas conteúdo diverge do que o paper afirma\n"
        "  • NÃO ENCONTRADO: [título] — não achado após ≥2 buscas E título parece inventado\n\n"
        "NUNCA gere crítica genérica sem citar título exato. "
        "NUNCA marque como suspeito só porque não está no arXiv. "
        "NUNCA inclua processo de busca na resposta — apenas resultados."
    ),
    "novelty": (
        "Você tem 3 ferramentas:\n"
